...
@section('title', 'Create Subscription Plan - Super Admin')
    ...
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="page-subtitle">Define a new subscription plan with features and limits</p>
            </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control @error('price_per_user') is-invalid @enderror"
                                   id="price_per_user" name="price_per_user" value="{{ old('price_per_user') }}" required>
                    ...
                    <div class="col-md-3">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('min_users') is-invalid @enderror"
                                   id="min_users" name="min_users" value="{{ old('min_users', 1) }}" min="1" required>
                            @error('min_users')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-3">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('max_users') is-invalid @enderror"
                                   id="max_users" name="max_users" value="{{ old('max_users') }}" min="1" required>
                            @error('max_users')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                                ...
                                <div class="card-body">
                                    <div class="form-check">
                                        <input class="form-check-input feature-checkbox" type="checkbox"
                                               ...
                                               value="1" {{ old("features.{$feature->id}.enabled") ? 'checked' : '' }}>
                                    ...
                                    @if($feature->is_quantity_based)
                                    <div class="feature-limits" style="display: {{ old("features.{$feature->id}.enabled") ? 'block' : 'none' }};">
                                        <label class="form-label small">Quantity Limit <span class="text-danger">*</span></label>
                                        <input type="number" class="form-control form-control-sm"
                                               name="features[{{ $feature->id }}][quantity_limit]"
                                               value="{{ old("features.{$feature->id}.quantity_limit") }}"
                                               placeholder="Enter limit" min="0" required>
                                        <small class="text-muted">Set the maximum allowed quantity</small>
                                    </div>
                                    @endif
                                </div>
                            </div>
