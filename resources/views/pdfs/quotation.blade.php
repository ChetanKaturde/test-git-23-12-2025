    ...
    <div class="page">
        ...
        <div class="document-header">
            ...
            <div class="document-details">
                <div><strong>Quote No:</strong> {{ $quotation->number ?? 'QUO-2526-0001' }}</div>
                <div><strong>Date:</strong> {{ isset($quotation->created_at) ? $quotation->created_at->format('d M Y') : now()->format('d M Y') }}</div>
                <div><strong>Valid Until:</strong> {{ isset($quotation->valid_until) ? $quotation->valid_until->format('d M Y') : now()->addDays(30)->format('d M Y') }}</div>
            </div>
        </div>
            ...
            <tbody>
                @if(isset($items) && count($items) > 0)
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <span class="currency">{{ $business->currency === 'INR' ? '₹' : '$' }}</span>{{ number_format($item->unit_price ?? 1000, 2) }}
                        </td>
        ...
        <div class="totals-section">
            <table class="totals-table">
                <tr>
                    ...
                    <td class="amount">
                        <span class="currency">{{ $business->currency === 'INR' ? '₹' : '$' }}</span>{{ number_format($quotation->subtotal ?? 10000, 2) }}
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td class="amount">
                        <span class="currency">{{ $business->currency === 'INR' ? '₹' : '$' }}</span>-{{ number_format($quotation->discount_amount ?? 0, 2) }}
                    </td>
                </tr>
                @endif
                <tr>
                    ...
                    <td class="amount">
                        <span class="currency">{{ $business->currency === 'INR' ? '₹' : '$' }}</span>{{ number_format($quotation->tax_amount ?? 1800, 2) }}
                    </td>
                </tr>
                <tr class="total-row">
                    <td class="label">Total Amount:</td>
                    <td class="amount">
                        <span class="currency">{{ $business->currency === 'INR' ? '₹' : '$' }}</span>{{ number_format($quotation->total ?? 11800, 2) }}
                    </td>
                </tr>
            </table>
        </div>
