<div class="max-w-6xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow p-6 mt-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- HSN/SAC Code -->
            <div id="hsn_sac_container">
                <label for="hsn_code" id="hsn_sac_label" class="block text-sm font-medium text-gray-700 mb-2">HSN Code</label>
                <input type="text" name="hsn_code" id="hsn_code" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('hsn_code', $material->hsn_code ?? '') }}" placeholder="e.g., 7208">
                <div class="text-red-500 text-sm mt-1" id="hsn_codeError"></div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleUnits() {
        ...
        // Toggle HSN/SAC label and placeholder
        const hsnLabel = document.getElementById('hsn_sac_label');
        const hsnInput = document.getElementById('hsn_code');
        if (isService) {
            hsnLabel.textContent = 'SAC Code';
            hsnInput.placeholder = 'e.g., 998314';
        } else {
            hsnLabel.textContent = 'HSN Code';
            hsnInput.placeholder = 'e.g., 7208';
        }
